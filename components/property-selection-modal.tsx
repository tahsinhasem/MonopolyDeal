...
export function PropertySelectionModal({
  ...
  const getAvailablePropertySets = () => {
    ...
    Object.entries(targetPlayer.properties).forEach(([color, cardIds]) => {
      ...
      if (
        actionType === "Sly Deal" ||
        (actionType === "Deal Breaker" && isComplete)
      ) {
        availableSets.push({
          color,
            ...
            <div className="text-center py-8">
              ...
              {actionType === "Deal Breaker" && (
                <p className="text-gray-400 text-sm mt-2">
                  Deal Breaker can only steal complete property sets.
                </p>
              )}
            </div>
          ) : (
                    ...
                    {propertySet.cards.map((cardId) => (
                      <div
                        ...
                        className={`cursor-pointer transition-all ${
                          selectedProperty === cardId
                            ? "ring-2 ring-red-500 transform scale-105"
                            : "hover:scale-102"
                        }`}
                        onClick={() =>
                          handlePropertySelect(cardId, propertySet.color)
                        }
                      >
                        <GameCard cardId={cardId} size="large" />
                        {actionType === "Sly Deal" && (
                          <p className="text-xs text-center mt-1 text-gray-600">
                            Click to steal this property
                          </p>
                        )}
                      </div>
                    ))}
                  ...
                  {actionType === "Deal Breaker" && propertySet.isComplete && (
                    <div className="mt-3 p-2 bg-orange-100 border border-orange-300 rounded">
                      <p className="text-sm text-orange-800 font-medium">
                        💥 Deal Breaker will steal ALL {propertySet.count}{" "}
            ...
            <div className="bg-red-50 border-2 border-red-200 rounded-lg p-4">
              ...
              <div className="flex items-center justify-center gap-4">
                <div className="text-center">
                  ...
                  <div className="mt-2">
                    <GameCard cardId={selectedProperty} size="medium" />
                  </div>
                  <p className="text-xs mt-1 font-medium capitalize">
                    {PROPERTY_COLORS[
                      selectedPropertyColor as keyof typeof PROPERTY_COLORS
                    ]?.name || selectedPropertyColor}
                  </p>
                </div>
                ...
                <div className="text-center">
                  ...
                  {actionType === "Deal Breaker" && (
                    <p className="text-xs text-orange-600 font-medium mt-1">
                      (Complete set of{" "}
                      {
                        availablePropertySets.find(
                          (s) => s.color === selectedPropertyColor
                        )?.count
                      }{" "}
                      properties)
                    </p>
