import { collection, doc, addDoc, getDoc, updateDoc, onSnapshot, query, where, increment } from "firebase/firestore"
  ...
  return new Promise((resolve) => {
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      ...
      const existingPlayer = Object.values(gameData.players).find(
        (player) => player.displayName.toLowerCase() === playerName.toLowerCase(),
      )
      ...
      // New player joining
      if (gameData.status !== "waiting" || Object.keys(gameData.players).length >= 5) {
        resolve({ gameId: null, isRejoining: false })
        return
      }
...
export async function executeGameAction(gameId: string, action: GameAction): Promise<void> {
  const gameRef = doc(db, "games", gameId)
  const gameSnap = await getDoc(gameRef)
  ...
  switch (action.type) {
    case "DRAW_CARDS":
      if (gameData.turnPhase === "draw" && gameData.cardsDrawnThisTurn === 0) {
        ...
        updates[`players.${action.playerId}.hand`] = [...player.hand, ...drawnCards]
    ...
    case "PLAY_MONEY":
      if (action.cardIds && gameData.turnPhase === "play" && gameData.cardsPlayedThisTurn < 3) {
    ...
    case "PLAY_PROPERTY":
      if (action.cardIds && action.propertyColor && gameData.turnPhase === "play" && gameData.cardsPlayedThisTurn < 3) {
        const cardId = action.cardIds[0]
        const card = getCard(cardId)

        if (card && card.type === "property" && player.hand.includes(cardId)) {
          const newHand = player.hand.filter((id) => id !== cardId)
          const currentProperties = player.properties[action.propertyColor] || []
          const newProperties = [...currentProperties, cardId]

          updates[`players.${action.playerId}.hand`] = newHand
          updates[`players.${action.playerId}.properties.${action.propertyColor}`] = newProperties
          updates.cardsPlayedThisTurn = gameData.cardsPlayedThisTurn + 1

          // Check for completed sets
          const colorInfo = PROPERTY_COLORS[action.propertyColor as keyof typeof PROPERTY_COLORS]
          if (colorInfo && newProperties.length === colorInfo.count) {
            updates[`players.${action.playerId}.completedSets`] = player.completedSets + 1
          }
        }
      }
      break
    ...
    case "DISCARD_CARDS":
      if (action.cardIds && gameData.turnPhase === "discard") {
        const newHand = player.hand.filter((id) => !action.cardIds!.includes(id))
        ...
        if (card && player.hand.includes(cardId)) {
          ...
          switch (card.name) {
            case "Pass Go":
              ...
              updates[`players.${action.playerId}.hand`] = [...newHand, ...passGoCards]
              ...
              Object.keys(gameData.players).forEach((playerId) => {
                if (playerId !== action.playerId) {
                  ...
                  if (payment > 0) {
                    ...
                    updates[`players.${playerId}.bankValue`] = newTargetBankValue
                    updates[`players.${action.playerId}.bank`] = newCurrentBank
                    updates[`players.${action.playerId}.bankValue`] = newCurrentBankValue
                  }
                }
              })
              break
              ...
              if (action.targetPlayerId) {
                ...
                if (targetPlayer) {
                  ...
                  if (payment > 0) {
                    ...
                    updates[`players.${action.targetPlayerId}.bank`] = newTargetBank
                    updates[`players.${action.targetPlayerId}.bankValue`] = newTargetBankValue
                    updates[`players.${action.playerId}.bank`] = newCurrentBank
                    updates[`players.${action.playerId}.bankValue`] = newCurrentBankValue
                  }
                }
            ...
            case "Wild Rent":
              if (action.targetPlayerId && action.propertyColor) {
                const targetPlayer = gameData.players[action.targetPlayerId]
                const currentPlayerProperties = player.properties[action.propertyColor] || []
                  ...
                  if (colorInfo) {
                    const rentAmount =
                      colorInfo.rentValues[
                        Math.min(currentPlayerProperties.length - 1, colorInfo.rentValues.length - 1)
                      ]
                    const payment = Math.min(rentAmount, targetPlayer.bankValue)
            ...
            case "Sly Deal":
              if (action.targetPlayerId && action.propertyColor) {
                const targetPlayer = gameData.players[action.targetPlayerId]
                if (targetPlayer && targetPlayer.properties[action.propertyColor]) {
                  const targetProperties = targetPlayer.properties[action.propertyColor]
                  if (targetProperties.length > 0) {
                    ...
                    const currentPlayerProperties = player.properties[action.propertyColor] || []
                    const newCurrentProperties = [...currentPlayerProperties, stolenProperty]

                    if (newTargetProperties.length === 0) {
                      // Remove the color entirely if no properties left
                      const newTargetPropertiesObj = { ...targetPlayer.properties }
                      delete newTargetPropertiesObj[action.propertyColor]
                      updates[`players.${action.targetPlayerId}.properties`] = newTargetPropertiesObj
                    } else {
                      updates[`players.${action.targetPlayerId}.properties.${action.propertyColor}`] =
                        newTargetProperties
                    }

                    updates[`players.${action.playerId}.properties.${action.propertyColor}`] = newCurrentProperties
                  }
                }
              }
              break
            ...
            case "Deal Breaker":
              if (action.targetPlayerId && action.propertyColor) {
                ...
                if (targetPlayer && targetPlayer.properties[action.propertyColor]) {
                  ...
                  if (colorInfo && targetProperties.length === colorInfo.count) {
                    // Steal the entire property set
                    const currentPlayerProperties = player.properties[action.propertyColor] || []
                    const newCurrentProperties = [...currentPlayerProperties, ...targetProperties]
                    ...
                    updates[`players.${action.targetPlayerId}.properties`] = newTargetPropertiesObj
                    updates[`players.${action.targetPlayerId}.completedSets`] = targetPlayer.completedSets - 1
                    updates[`players.${action.playerId}.properties.${action.propertyColor}`] = newCurrentProperties
            ...
            case "Forced Deal":
              if (action.targetPlayerId && action.cardIds && action.cardIds.length === 3) {
                const targetPlayer = gameData.players[action.targetPlayerId]
                const [forcedDealCardId, myPropertyId, targetPropertyId] = action.cardIds
                ...
                if (targetPlayer) {
                  ...
                  if (myPropertyColor && targetPropertyColor) {
                    ...
                    const myCurrentProperties = player.properties[myPropertyColor] || []
                    const myNewProperties = myCurrentProperties.filter((id) => id !== myPropertyId)

                    // Remove target property from target's collection
                    const targetCurrentProperties = targetPlayer.properties[targetPropertyColor] || []
                    const targetNewProperties = targetCurrentProperties.filter((id) => id !== targetPropertyId)
                    ...
                    if (myNewProperties.length === 0) {
                      ...
                      updates[`players.${action.playerId}.properties`] = newMyProperties
                    } else {
                      updates[`players.${action.playerId}.properties.${myPropertyColor}`] = myNewProperties
                    }
                    updates[`players.${action.playerId}.properties.${targetPropertyColor}`] = myNewTargetColorProperties

                    // Update target's properties
                    if (targetNewProperties.length === 0) {
                      const newTargetProperties = { ...targetPlayer.properties }
                      delete newTargetProperties[targetPropertyColor]
                      updates[`players.${action.targetPlayerId}.properties`] = newTargetProperties
                    } else {
                      updates[`players.${action.targetPlayerId}.properties.${targetPropertyColor}`] =
                        targetNewProperties
                    }
                    updates[`players.${action.targetPlayerId}.properties.${myPropertyColor}`] =
                    ...
                    const myUpdatedProperties = {
                      ...
                      ...(myNewProperties.length === 0 ? {} : { [myPropertyColor]: myNewProperties }),
                    }
                    if (myNewProperties.length === 0) {
                      delete myUpdatedProperties[myPropertyColor]
                    }

                    Object.entries(myUpdatedProperties).forEach(([color, cardIds]) => {
                    ...
                    const targetUpdatedProperties = {
                      ...
                      ...(targetNewProperties.length === 0 ? {} : { [targetPropertyColor]: targetNewProperties }),
                    }
                    if (targetNewProperties.length === 0) {
                      delete targetUpdatedProperties[targetPropertyColor]
                    }

                    Object.entries(targetUpdatedProperties).forEach(([color, cardIds]) => {
                      const colorInfo = PROPERTY_COLORS[color as keyof typeof PROPERTY_COLORS]
                      if (colorInfo && cardIds.length === colorInfo.count) {
                        targetCompletedSets++
                      }
                    ...
                    updates[`players.${action.playerId}.completedSets`] = myCompletedSets
                    updates[`players.${action.targetPlayerId}.completedSets`] = targetCompletedSets
                  }
                }
              }
              break
